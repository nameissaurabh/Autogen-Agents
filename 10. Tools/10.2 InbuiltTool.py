...
openai_client = OpenAIChatCompletionClient(model = "gemini-2.0-flash", 
                                           api_key = api_key)
...
async def main():
    ...
    assistant = AssistantAgent(
        ...
        system_message = "You are a helpful assistant that can fetch random cat facts"
                         "(fdirectly call the tool, no changes/inputs) and reverse strings using Tools."
        )

    # The assistant can now use the base64 tool to decode the string
    response = await assistant.on_messages(
        [TextMessage(content = "Can you please fetch a cat fact using the tool?.",
                     source = "user")],
        CancellationToken(),
    )
